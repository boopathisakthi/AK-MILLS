...
module.exports = {
    ...
    delete(req, res) {
        Payment.find({ "PaymentDetail.trans_id": req.params._id, isdeleted: 0 })
            .then((paymentdata) => {
                if (paymentdata.length != 0) {
                    ...
                    return res.status(404).send("Can't Delete this Invoice " + paymentdata[0].PaymentDetail[0].trans_no + " there is a Receipt(" + billno + ")  aganist this Invoice.. pls delete that");

                }
                else {
                    sales.findOne({ _id: req.params._id, branchid: new mongoose.Types.ObjectId(req.session.branchid) })
                        .then(data => {
                            if (!data) {
                                return res.status(404).send({
                                    message: 'Data Not Found',
                            ...
                            Promise.resolve(data.invoiceDetail).then(each((ele) => {
                                ...
                                StockProcessDetails.findOne({ isdeleted: 0, productid: ele.productid, branchid: req.session.branchid })
                                    .then((stockdata) => {
                                        ...
                                        stockdata.updateOne({
                                            salesqty: parseInt(stockdata.salesqty) - parseInt(ele.qty)
                                        }).then((data1) => { })
                                    })
